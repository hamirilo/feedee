...
async def run_migration(session: AsyncSession):
    ...
    try:
        ...
        # --- Migrate Categories ---
        cur.execute(
            "SELECT id, name, color, content_type, display_order, user_id FROM rssapp_category"
        )
        ...
        for c in sqlite_categories:
            ...
            stmt = select(Category).where(
                Category.user_id == pg_user_uuid, Category.name == c["name"]
            )
            res = await session.execute(stmt)
            ...
            if not pg_cat:
                ...
                pg_cat = Category(
                    user_id=pg_user_uuid, name=c["name"], color=c["color"] or "#6b7280", scope=scope
                )
                session.add(pg_cat)
                await session.flush()
        ...
        cur.execute(
            "SELECT id, name, url, display_order, is_active, category_v2_id FROM rssapp_feed"
        )
        ...
        for f in sqlite_feeds:
            ...
            if pg_user_uuid:
                ...
                stmt = select(Subscription).where(
                    Subscription.user_id == pg_user_uuid, Subscription.feed_id == pg_feed.id
                )
                res = await session.execute(stmt)
        ...
        # --- Migrate Bookmarks ---
        cur.execute(
            "SELECT id, url, title, description, thumbnail_url, category_v2_id, user_id FROM rssapp_bookmark"
        )
        ...
        for b in sqlite_bookmarks:
            ...
            stmt = select(Bookmark).where(
                Bookmark.user_id == pg_user_uuid, Bookmark.url == b["url"]
            )
            res = await session.execute(stmt)
            ...
            if not pg_bm:
                ...
                state = BookmarkUserState(
                    user_id=pg_user_uuid, bookmark_id=pg_bm.id, is_pinned=False, is_favorited=False
                )
                session.add(state)
